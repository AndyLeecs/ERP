...
public class StockReturnListBLServiceImpl extends SaleUniBLServiceImpl
{
	...
	public SalesmanListPO voToPo(SalesmanListVO vo) {
		...
		return new StockReturnListPO(svo.getId(),svo.getState(),DateUtil.getDateFromListID(svo.getId()),svo.getMemberID(),svo.getMemberName(),svo.getOperator(),svo.getOperatorId(),svo.getRealOperator(),svo.getWarehouse(),svo.getNotes(),polist,svo.getSum());
	}
	...
	public SalesmanListVO poToVo(SalesmanListPO po) {
		...
		return new StockReturnListVO(spo.getId(), spo.getOperator(), spo.getOperatorId(), spo.getState(), spo.getMemberID(), spo.getMemberName(),spo.getRealOperator(), spo.getWarehouse(), spo.getNotes(), volist, spo.getSum());
	}
